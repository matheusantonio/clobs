    ...
    <div class="shadow border">
        <div class="m-4">
            <div class="m-2"><h2>
                ...
                Search Results for "{{tag}}"</h2></div>
            <div class="list-group list-group-flush">
                <div v-for="bookmark in searchResults" :key="bookmark.id" class="list-group-item d-flex justify-content-between align-items-center">
                    <Bookmark :name="bookmark.name" :url="bookmark.url" :tags="bookmark.tags"/>
...
export default {
    ...
    components : {Bookmark},
    methods : {
        searchBookmarks(tag, limit, offset) {
            Index.searchByTag(tag, limit, offset, (response) => {
    ...
    mounted : function() {
        this.searchBookmarks(this.tag, 10, 0)
    },
    beforeRouteUpdate(to, from, next) {
        this.searchResults = []
        this.searchBookmarks(to.params.tag, 10, 0)
        next()
    }
